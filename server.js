...
app.get('/song', (req, res) => res.sendFile(path.join(__dirname, 'public', 'song.html')));
...
app.post('/api/events/:id/song/clear', (req, res) => {
  ...
  io.to(`event:${event.id}`).emit('display_mode_changed', { mode: 'auto', manualSource: event.displayState.manualSource, manualTranslations: event.displayState.manualTranslations, updatedAt: event.displayState.updatedAt });
  res.json({ ok: true, event: normalizeEvent(event) });
});
