...
function renderHeroActiveEventSelect(activeEventId) {
  ...
  if (!select) return;
  // FIX-ACTIVE-EVENT-3 — selectăm evenimentul DESCHIS (currentEvent) ca să nu "sară" înapoi la
...
const SPEED_HINTS = {
  ...
  interpret: 'Parafrazează concis ca un interpret uman — păstrează sensul, dar ține pasul cu un vorbitor rapid sau continuu.'
};
...
document.getElementById('heroGoLiveBtn')?.addEventListener('click', async () => {
  ...
  // FIX-ACTIVE-EVENT-3 — dacă e programat în viitor, întreabă dacă pornim instant (skip countdown)
...
// WORSHIP-ROLES-1B: add/update (POST upsert) + delete + click-pe-rând pentru editare (prefill)
document.getElementById('addWorshipRoleBtn')?.addEventListener('click', async () => {
  ...
  const canManageRoles = !!document.getElementById('worshipRoleCanManage')?.checked;   // WORSHIP-MANAGE-ROLES
  if (!name) return;
  try {
    ...
    ['worshipRoleCanLead', 'worshipRoleCanAdmin', 'worshipRoleCanManage'].forEach((id) => { const el = document.getElementById(id); if (el) el.checked = false; });
...
document.getElementById('worshipRolesList')?.addEventListener('click', async (e) => {
  ...
  if (delBtn) {
    const name = delBtn.getAttribute('data-worship-role-delete');
  ...
  const manageEl = document.getElementById('worshipRoleCanManage');   // WORSHIP-MANAGE-ROLES
  ...
  if (manageEl) manageEl.checked = item.getAttribute('data-role-manage') === '1';
});
...
$('eventList').addEventListener('click', async (e) => {
  ...
  if (action === 'rename') {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!data.ok) { alert(data.error || 'Redenumire eșuată.'); return; }
      await refreshEventList();
    } catch (err) {
